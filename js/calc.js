/**
 * Created by DelMonroe on 4/17/17.
 */
// (function () {
    "use strict";

    var val = [];
    function btn_1() {

        var b1 = document.getElementById("I-1");
        b1.setAttribute("value", "1");
         var res = (b1.getAttribute("value"));

        return val;
    }
    val.push(btn_1());
    console.log(val);

    var btn1 = document.getElementById("btn_1");
    btn1.addEventListener("click", btn_1);




    function btn_2() {
        var b2= document.getElementById("I-1");
        b2.setAttribute("value", "2");
        val = b2.getAttribute("value");
        return val;
    }
    var btn2 = document.getElementById("btn_2");
    btn2.addEventListener("click", btn_2);

    function btn_3() {
        var b3 = document.getElementById("I-1");
        b3.setAttribute("value", "3");
    }
    var btn3 = document.getElementById("btn_3");
    btn3.addEventListener("click", btn_3);

    function btn_4() {
        var b4 = document.getElementById("I-2");
        b4.setAttribute("value", "+");

    }
    var btn4 = document.getElementById("btn_4");
    btn4.addEventListener("click", btn_4);

    function btn_5() {
        var b1 = document.getElementById("I-1");
        b1.setAttribute("value", "4");

    }

    function btn_6() {
        var b1 = document.getElementById("I-1");
        b1.setAttribute("value", "4");

    }

    function btn_7() {
        var b1 = document.getElementById("I-1");
        b1.setAttribute("value", "4");

    }

    function btn_8() {
        var b1 = document.getElementById("I-1");
        b1.setAttribute("value", "4");

    }

    function btn_9() {
        var b1 = document.getElementById("I-1");
        b1.setAttribute("value", "4");

    }

    function btn_10() {
        var b1 = document.getElementById("I-1");
        b1.setAttribute("value", "4");

    }

    function btn_11() {
        var b1 = document.getElementById("I-1");
        b1.setAttribute("value", "4");

    }

    function btn_12() {
        var b1 = document.getElementById("I-1");
        b1.setAttribute("value", "4");

    }

    function btn_13() {
        var b1 = document.getElementById("I-1");
        b1.setAttribute("value", "4");

    }

    function btn_14() {
        var b1 = document.getElementById("I-1");
        b1.setAttribute("value", "4");

    }

    function btn_15() {
        var b1 = document.getElementById("I-1");
        b1.setAttribute("value", "4");

    }

    function btn_16() {
        var b1 = document.getElementById("I-1");
        b1.setAttribute("value", "4");

    }

// })();